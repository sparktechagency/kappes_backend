...
const createOffered = async (offerProductsData: ICreateOfferedInput, user: IJwtPayload) => {
     ...
     if (isExistsProducts.length !== offerProductsData.products.length) {
          throw new AppError(StatusCodes.BAD_REQUEST, `Some products not found ${offerProductsData.products.join(', ')} in db`);
     }

     const isProductOwner = isExistsProducts.every((product: any) => product.shopId.owner.toString() === user.id.toString());

     const isProductAdmin = isExistsProducts.every((product: any) => product.shopId.admins?.some((admin: any) => admin.toString() === user.id.toString()));

     if (!isProductOwner && !isProductAdmin) {
          throw new AppError(StatusCodes.FORBIDDEN, `You have no permission to create offered for this product`);
     }
     const { products, discountPercentage } = offerProductsData;
     const createdBy = user.id;

     const operations = products.map((product) => {
          const productObjectId = new mongoose.Types.ObjectId(product); // ✅ CHANGE

          return {
               updateOne: {
                    filter: { product: productObjectId },
                    update: {
                         $setOnInsert: {
...
const getActiveOfferedService = async (query: Record<string, unknown>) => {
     ...
     const offeredQuery = new QueryBuilder(Offered.find().populate('product', 'name basePrice'), query).paginate();
...
const getAllOffered = async (query: Record<string, unknown>) => {
     const offeredQuery = new QueryBuilder(Offered.find().populate('product', 'name basePrice'), query).paginate().filter().sort();
