...
const verifyEmailToDB = async (payload: IVerifyEmail) => {
     ...
     const isExistBusiness = await Business.findOne({ email }).select('+authentication').lean();
     if (!isExistBusiness) {
          throw new AppError(StatusCodes.BAD_REQUEST, "Business doesn't exist!");
     }
     ...
     if (!isExistBusiness.verified) {
          ...
          accessToken = jwtHelper.createToken(
               { id: isExistBusiness._id.toString() as string, role: '', businessType: isExistBusiness.type, email: isExistBusiness.email },
...
const searchBusinesses = async (params: IBusinessSearchParams, userId?: string): Promise<IBusiness[]> => {
