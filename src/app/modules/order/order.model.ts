...
import { DAY_FOR_DELIVERY_OPTIONS, DELIVERY_OPTIONS, DeliveryPlatformEnum, EXTRA_DELIVERY_COST_PERCENT_FOR_DELIVERY_OPTIONS, ORDER_STATUS, PAYMENT_METHOD, PAYMENT_STATUS } from './order.enums';
...
const orderSchema = new Schema<IOrder>(
     {
          ...
          deliveryPlatForm: {
               type: String,
               enum: Object.values(DeliveryPlatformEnum),
          },
     },
     {
          timestamps: true,
...
orderSchema.pre('validate', async function (next) {
     ...
     if (order.deliveryPlatForm == DeliveryPlatformEnum.CHITCHAT) {
          if (order.chitchats_postage_type) {
               ...
               deliveryCharge = Number(chitChatsDeliveryCharge);
          } else {
               throw new AppError(StatusCodes.BAD_REQUEST, 'Chitchat postage type is required for Chitchat delivery platform');
          }
     } else if (order.deliveryPlatForm == DeliveryPlatformEnum.SHIPSTATION) {
          throw new AppError(StatusCodes.BAD_REQUEST, 'shipstation is not implemented yet');
     }
